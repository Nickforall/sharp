...
/**
 * Keep existing ICC profile instead of overwriting it to sRGB
 *
 * @example
 * sharp('input.jpg')
 *  .keepExistingProfile()
 *  .toFile('output-with-icc-profile.jpg')
 *  .then(info => { ... });
 *
 *  @param {Boolean} [keepExistingProfile] Keep existing (input) ICC profile or override it to sRGB.
 *  @returns {Sharp}
 */
function keepExistingProfile (keepExistingProfile) {
  this.options.keepExistingProfile = is.bool(keepExistingProfile) ? keepExistingProfile : true;
  return this;
}
